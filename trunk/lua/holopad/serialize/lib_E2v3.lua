...
//if (holoRemainingSpawns() < TotalHolos) {error("This model has too many holos to spawn! (" + TotalHolos + " holos needed but only " + holoRemainingSpawns() + " available!)")}
local part3 = 
[[    
    ##########
    
    TotalHolos = Holos:count()
    if (%i > holoClipsAvailable()) {error("A holo has too many clips to spawn on this server! (Max per holo is " + holoClipsAvailable() + ")")}
}
...
function lib.Save( modelobj, filename, overwrite, options )
	...
	if (!overwrite && file.Exists(path, "DATA")) then Error("Unable to export to E2; \"" .. path .. "\" already exists!\n") return false end
...
local function parentLocal(child, parent, scale)
	return WorldToLocal(child.pos*scale, child.ang, parent.pos*scale, parent.ang)
end
...
function lib.tablesToE2(tables, options)
	...
	local maxclips = 0
	local listmap = {}
	local cur, clips, clip, clipno, parent, plpos, plang
	for i=1, #hololist do
		cur = hololist[i]
		listmap[cur] = i
		parent = uidmap[cur.parent] or {pos = Vector(0,0,0), ang = Angle()}
		plpos, plang = parentLocal(cur, parent, scale)
		ret.add(holodef(i, plpos, cur.scale*scale, cur.colour, plang, cur.model, cur.material, listmap[uidmap[cur.parent]], cur.name))
		...
		if clips then
			...
			for j=1, clipno do
				clip = clips[j]
				plpos, plang =  parentLocal(clip, cur, scale)
				ret.add(clipdef(listmap[uidmap[clip.parent]], j, plpos, plang:Up()))
			end
		end
	end
