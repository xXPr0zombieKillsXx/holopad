/**
	...
	By Bubbus			 | | FlipScaleXY Context Control Derma
...
function PANEL:Init()
	...
	self.PropSheet:AddSheet( "Tool Config", self:createControls(), "holopad/tools/squashface", false, false, "Tool Config" )
...
function PANEL:SetTool(mdl)
	...
	if !mdl:class() == Holopad.Tools.FlipScaleXY then Error("Can't bind this gui to this tool!") return end
	self.tool = mdl
end
...
function PANEL:createControls()	
	...
	collabel:SetText(
[[This is the GM12 Scale Fix Tool!

Press the button to fix the GM12 holos.
(the scale X and Y values will be swapped)
...
(just press the button again!)
	...
	selectbutton:SetText( "Fix GM12 Holos" )
	selectbutton.DoClick = apply
	selectbutton:SetSize(180, 20)
	selectbutton:SetPos(5, ypos)
	
	ypos = ypos + 10 + selectbutton:GetTall()
...
derma.DefineControl( "DFlipScaleXY_Holopad", "Context controls for FlipScaleXY Tool", PANEL, "DFrame" )
